...
from scrappers.arc.arc import ARCScrapper, ARC_V1
...
class ARCScrapperV1(ARCScrapper, version=ARC_V1):
            ...
            for race_id in [r for r in race_ids if r not in self._excluded_ids]:
                ...
                for table in tables:
                    ...
                    for row in rows:
                        ...
                        yield ScrappedItem(
                            name=name, trophy_name=trophy_name, town=None, league=league, gender=None,
                            organizer=None, edition=edition, day=day, t_date=t_date,
                            club_name=club_name, participant=self.normalized_club_name(club_name),
                            series=series, lane=self.get_lane(row), laps=self.get_laps(row),
                            race_id=race_id, url=url, datasource=self.DATASOURCE,
                        )
    ...
    def get_summary_soup(self, group: str = None, **kwargs) -> Tuple[Tag, str]:
    ...
    def get_league(self, group: str, **kwargs) -> Optional[str]:
        match group:
            case 'playoffact':
    ...
    def get_name(self, soup: Tag, race_id: str = None, **kwargs) -> str:
    ...
    def get_date(self, soup: Tag, race_id: str = None, **kwargs) -> date:
        rows = [row.find('a') for row in soup.find_all('div', {'class': 'listado_regatas_nombre'})]
