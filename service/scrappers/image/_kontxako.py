...
from scrappers import ScrappedItem
from scrappers.image.image import ImageScrapper, IMAGE_KONTXAKO
from utils.synonyms import TIME_TRIAL_SYNONYMS, FEMALE_SYNONYMS, CLASSIFICATION_SYNONYMS

logger = logging.getLogger(__name__)


class ImageScrapperKontxako(ImageScrapper, source=IMAGE_KONTXAKO):
    DATASOURCE = IMAGE_KONTXAKO

    def scrap(self, path: str, optimize: bool = False, **kwargs) -> List[ScrappedItem]:
        ...
        details = self.get_race_details_soup(img_bin=img_bin)
        if not self.get_name(details) or not self.get_date(details):
            details = self.get_race_details_soup(img_bin=img_bin, optimize=True)
        ...
        df = self.get_summary_soup(img=img, img_vh=img_vh, optimize=optimize)
        ...
        for _, row in df.iterrows():
            ...
            yield ScrappedItem(
                name=name, trophy_name=self.normalized_name(name), edition=1, day=1, t_date=t_date,
                town=town, gender=gender, league=None, organizer=organizer,
                club_name=club_name, participant=self.normalized_club_name(club_name),
                series=(itx // race_lanes) + 1, lane=self.get_lane(row), laps=self.get_laps(row),
                race_id=os.path.basename(path), url=None, datasource=self.DATASOURCE,
                race_laps=race_laps, race_lanes=race_lanes
            )
            itx += 1
