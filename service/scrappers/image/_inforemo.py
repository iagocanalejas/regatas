...
from scrappers.image.image import ImageScrapper, IMAGE_INFOREMO
...
# noinspection Assert
class ImageScrapperInforemo(ImageScrapper, source=IMAGE_INFOREMO):
    ...
    def scrap(self, path: str, optimize: bool = False, **kwargs) -> List[ScrappedItem]:
        ...
        for itx, row in df.iterrows():
            ...
            yield ScrappedItem(
                name=name, trophy_name=trophy_name, edition=1, day=1, t_date=t_date,
                town=town, gender=self.get_gender(row), league=None, organizer=None,
                club_name=club_name, participant=self.normalized_club_name(club_name),
                series=self.get_series(row), lane=self.get_lane(row), laps=self.get_laps(row),
