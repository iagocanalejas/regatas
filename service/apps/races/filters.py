    ...
    def build_query(self) -> QuerySet[Race]:
        ...
        if self.keywords:
            queryset = queryset.filter(
                Q(trophy__isnull=False, trophy__name__unaccent__icontains=self.keywords)
                | Q(flag__isnull=False, flag__name__unaccent__icontains=self.keywords)
                | Q(league__isnull=False, league__name__unaccent__icontains=self.keywords)
                | Q(sponsor__isnull=False, sponsor__unaccent__icontains=self.keywords)
                | Q(town__isnull=False, town__unaccent__icontains=self.keywords)
            )
