...
class ParticipantFilters:
    ...
    def sort_by_speed(queryset: QuerySet[Participant], reverse: bool) -> List[Participant]:
        # in-memory sort
        elements = list(
            queryset.filter(laps__len__gt=0)
            .annotate(time=RawSQL("participant.laps[array_length(participant.laps, 1)]", ()))
            .all()
        )
        elements.sort(
            key=lambda p: p.distance / ((p.time.minute * 60) + p.time.second + (p.time.microsecond / 100000)),
            reverse=reverse,
        )
        return elements
    ...
    def build_query(self) -> QuerySet[Participant]:
        ...
        if self.keywords:
            queryset = queryset.filter(
                Q(race__trophy__isnull=False, race__trophy__name__icontains=self.keywords)
                | Q(race__flag__isnull=False, race__flag__name__icontains=self.keywords)
                | Q(race__league__isnull=False, race__league__name__icontains=self.keywords)
                | Q(race__sponsor__isnull=False, race__sponsor__icontains=self.keywords)
                | Q(race__town__isnull=False, race__town__icontains=self.keywords)
            )
